...
func TestYAMLFormatter_Healthy(t *testing.T) {
	...
	if !strings.Contains(outputStr, "kind: http") {
		t.Errorf("expected 'kind: http' in YAML output: %s", outputStr)
	}
...
func TestJUnitFormatter_DNSDetails(t *testing.T) {
	dnsDetail := &details.Detail_DNS{
		Host:      "api.example.com",
		Server:    "8.8.8.8:53",
		QueryType: "A",
		Records: []*details.DNSRecord{
	...
	anyDetail, err := anypb.New(dnsDetail)
	if err != nil {
		t.Fatalf("failed to create any: %v", err)
	}
	...
	// Verify DNS details in system-out
	if !strings.Contains(outputStr, "DNS Query:") {
		t.Error("expected DNS Query header in output")
	}
	if !strings.Contains(outputStr, "api.example.com") {
		t.Error("expected host in output")
	}
	if !strings.Contains(outputStr, "192.168.1.10") {
		t.Error("expected A record value in output")
	}
	if !strings.Contains(outputStr, "DNSSEC: enabled, authenticated") {
		t.Error("expected DNSSEC status in output")
	}
}
