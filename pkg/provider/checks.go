...
// EvaluateChecks runs all checks against the provided CEL context.
...
func (b *BaseWithChecks) EvaluateChecks(ctx context.Context, celCtx map[string]any) []string {
	return b.EvaluateChecksWithBindings(ctx, celCtx)
}

// EvaluateChecksWithBindings runs all checks with optional runtime program options.
// This is used to inject runtime function bindings (e.g., kubernetes.Get with K8s client).
// Returns nil if all expressions pass, or a slice of failure messages.
// If fail-fast mode is enabled in the Go context, returns immediately after first failure.
func (b *BaseWithChecks) EvaluateChecksWithBindings(ctx context.Context, celCtx map[string]any, opts ...cel.ProgramOption) []string {
	if len(b.compiled) == 0 {
		return nil
	}
	...
	for _, check := range b.compiled {
		msg, err := check.EvaluateWithBindings(celCtx, opts...)
		if err != nil {
